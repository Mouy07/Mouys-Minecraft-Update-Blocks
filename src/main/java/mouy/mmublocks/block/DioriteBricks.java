...
public class DioriteBricks extends Block {
    ...
    public DioriteBricks(Settings settings) {
        super(settings);
    }

    static final String BLOCK_ID = "diorite_bricks";
    public static final Block DIORITE_BRICKS = new Block(FabricBlockSettings.copyOf(Blocks.DIORITE));

    public static void registerBlock() {
        Registry.register(Registries.BLOCK, new Identifier(MouysMinecraftUpdateBlocks.MOD_ID, BLOCK_ID), DIORITE_BRICKS);
        registerBlockItem(DIORITE_BRICKS);
    }
    ...
    public static void addToItemGroup(Item blockItem) {
        ItemGroupEvents.modifyEntriesEvent(ItemGroups.BUILDING_BLOCKS).register(content -> content.addAfter(Items.POLISHED_DIORITE_SLAB, blockItem));
    }
}
