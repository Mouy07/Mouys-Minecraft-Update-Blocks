...
public class PolishedCalcite extends Block {

    public PolishedCalcite(Settings settings) {
        super(settings);
    }

    static final String BLOCK_ID = "polished_calcite";
    public static final Block POLISHED_CALCITE = new Block(FabricBlockSettings.copyOf(Blocks.CALCITE));

    public static void registerBlock() {
        Registry.register(Registries.BLOCK, new Identifier(MouysMinecraftUpdateBlocks.MOD_ID, BLOCK_ID), POLISHED_CALCITE);
        registerBlockItem(POLISHED_CALCITE);
    }
    ...
    public static void addToItemGroup(Item blockItem) {
        ItemGroupEvents.modifyEntriesEvent(ItemGroups.BUILDING_BLOCKS).register(content -> content.addAfter(Items.CALCITE, blockItem));
    }
}